...
# 何も工夫をしないとCriticalを経由せずに Warning -> OK になった場合もエアコンをOFFにしてしまうのでうっとうしい。
# しかしながらMackerelのAPIは
...
# そのため今は適当な時間帯にOKになった場合は無視するようにしている。
#
# ちゃんとやるなら、起床時にアラートを閉じてしまうような仕組みをつくればとりあえず解決はできそう。
#
# もっとちゃんとやるなら、そもそもエアコンのON/OFF状態をまともに取得する手段は存在しないため(純正リモコンで操作すれば途端に分からなくなる)、
